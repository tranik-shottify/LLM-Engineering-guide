# Prompt — Volume 1 of 3

Paste this entire message into a fresh claude.ai conversation. This is
**Volume 1 of a 3-volume reference document on LLM engineering**. Each
...
**Parts 1-5** — Foundations refresher, Modern model landscape, Adapting
models (SFT / PEFT / alignment / distributed training), Retrieval-Augmented
...
- **Target length when printed: 28-34 A4 pages** for this volume.
- Renders correctly when opened in Chrome and printed via "Print → Save
  as PDF". Page-breaks, headers, footers, and figure placement must
  behave under print CSS.
...
- **Page footer**: page number `counter(page) / counter(pages)` centered;
  small "Volume 1 of 3" on the left.
- **Cover page** (page 1, no header/footer):
  ...
  - Subtitle directly below: *Volume 1 · Foundations · Models · Adaptation
    · Retrieval · Agents*, **16 pt**, weight 400, muted color `#4A5568`.
...
- **Volume contents** (page 2): list of the five Parts in this volume only,
  ...
  listing — by name only, no page numbers — what Volume 2 (Parts 6-10:
  Inference, Production, Evaluation, Safety, MLOps & LLMOps) and Volume 3
  (Parts 11-15: Multimodal, System Design, Trends, Resources, Glossary)
  contain.
...
to that width. A previous draft of this document failed exactly that
way; do not repeat the mistake.
...
- **Do not unroll repeating structures.** For "× N decoder blocks" or
  ...
  annotation on the subgraph border. Drawing the same shape eight
  times wastes page width and crushes text into illegibility.
- **Use `subgraph` to group logically.** A pipeline with 3-5 stages
  ...
  nodes — not one long unbroken chain.
...
- After drafting each diagram, mentally render it at ~174 mm wide and
  ...
  below ~12 px effective size, redesign — reduce nodes, switch to TB,
  or abstract the repeating segment.
...
- **Never put math inside `<code>` or `<pre>`** — MathJax skips those
  ...
  text. This is the most common cause of "raw LaTeX showing up on the
  page".
- **Do not place math inside a markdown fenced code block.** Use plain
  HTML containers around the delimiters.
- **Each display equation is a single logical unit.** The wrapping
  `<div>` must be wide enough to hold the full expression without
  forcing a hard line break inside the TeX source.
...
- **Figure 7** (GraphRAG / RAPTOR architecture) — `flowchart TB` with
  subgraphs separating ingestion, indexing, and retrieval stages.
- **Figure 9** (multi-agent state graph) — `flowchart TB`; one node per
  agent role; edges labeled with hand-offs and conditions.

## 4. Content — what each Part must contain

### Part 1 — Foundations refresher *(brief but complete, ~6-7 pages)*
...
### Part 2 — Modern model landscape (2025-2026) *(~3 pages)*
...
### Part 3 — Adapting models: SFT, PEFT, alignment, distributed training *(~6 pages)*
...
- Include a **15-line code snippet**: minimal LoRA + SFT setup with
  HuggingFace `peft` and `trl` (`SFTTrainer`).

### Part 4 — Retrieval-Augmented Generation, beyond v1 *(~6 pages)*
...
### Part 5 — Agentic systems *(~6 pages)*
...
10. **Every Part must open with a one-sentence italic deck line** under
    its title.
11. **This is Volume 1 of 3**. Render only the Parts listed in this
    prompt. Do not generate content for Parts 6-15.
...
3. Confirm Figures 1-10 are present and correctly numbered.
